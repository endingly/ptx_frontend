...
这一分支用于在没有 indirect-call descriptor 时提供更清楚的诊断，当前可以接受，但它不是最终结构。M7 完成后必须删除该临时分支，使 indirect call 通过正式 descriptor、binding 和 semantic contract 进入统一流程。
...
M7-I01/I02/I03/I04 已完成：function-local `.callprototype`、`.calltargets` 与
...
现在以稳定的 function-scope SymbolId 进入 binding；metadata declaration semantics、Resolved
IR 与 indirect-call descriptor 尚未实现，其余 M7 issue 仍未完成。
