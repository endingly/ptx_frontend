...
- module/function symbol scopes, reference classification, and declaration
  semantics;
- generated resolution and checking for YAML-modelled bare `ret`/`exit`/`trap` (with ordinary predicate guards accepted), fixed `and.b32`/`or.b32`/`xor.b32`/`not.b32`/`shl.b32`/`shr.u32`, frozen `setp.lt{.and}.u32` single-predicate forms and `selp.u32`, `bra`, `brx.idx`, `add`,
