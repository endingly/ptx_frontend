...
| Structured control syntax | Supported subset | `.callprototype` has dedicated function-local CST/AST syntax with its PTX 9.3 signature/suffix payload; `.calltargets` has dedicated function-local CST/AST syntax with ordered targets (including retained duplicates); nested scopes, `.branchtargets`, and all metadata binding, declaration semantics, resolution, and control-flow checking remain unsupported |
