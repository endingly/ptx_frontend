...
| Resolved opcodes | Partial | The machine-readable manifests are authoritative for modelled opcode slices and deferred scope. The M12 common-kernel corpus validates 60 frozen forms through parse, resolve, and target-aware checking on `sm_80`, `sm_90a`, and `sm_100`; `setmaxnreg.inc.sync.aligned.u32` is intentionally `sm_90a`-only. Inventory entries remain partial for implemented frozen slices with residual variants deferred after M12; simulator execution remains unsupported. |
