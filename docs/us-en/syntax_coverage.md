...
| Extended-precision add/sub | Supported | `add.cc`, `addc{.cc}`, `sub.cc`, and `subc{.cc}` expose typed carry/borrow effects and target-aware validation; see [carry coverage](carry_coverage.md). Runtime CC state remains outside the frontend |
