...
| Recovery/editing | Foundation only | Public parser/lowering results carry an ordered `DiagnosticCollection`; parsing remains fail-fast with one diagnostic until missing tokens, recovery nodes, synchronization, multi-error parsing, and token edits arrive |
