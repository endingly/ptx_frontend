...
| Structured control syntax | Supported subset | `.callprototype` has dedicated function-local CST/AST syntax with its PTX 9.3 signature/suffix payload; nested scopes, `.calltargets`/`.branchtargets`, and all metadata binding, declaration semantics, resolution, and control-flow checking remain unsupported |
