...
| Resolved opcodes | Partial | Only opcodes present in the YAML database; currently `add`, `sub`, `bar`, `bra`, register/immediate/special-register sources for `mov.u32/.u64`, data-symbol/symbol-plus-offset sources for `mov.u64`, and generic `ld.u32 d, [address]`, retaining binding-aware identity and corresponding type/target checks |
...
1. Complete `mov` function/parameter addresses, remaining scalar/vector types,
   and special-register type-width forms backed by explicit historical rules.
