...
| Recovery/editing | Foundation only | Public parser/lowering results carry an ordered `DiagnosticCollection`; CST models inserted/skipped/error recovery nodes, but the parser does not emit them yet. Parsing remains fail-fast with one diagnostic until synchronization, multi-error parsing, and token edits arrive |
