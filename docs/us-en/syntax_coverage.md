...
| Debug file directive | Supported subset | Outermost `.file file_index "filename"` with optional paired `, timestamp, file_size`; CST is lossless and AST retains field ranges, while duplicate-index and `.loc` tables remain unsupported |
...
| Other directives | Not supported (rejected) | `.loc`, section, pragma, module variable, and structured kernel-tuning directives; unmodeled function-header tokens never silently enter the AST |
