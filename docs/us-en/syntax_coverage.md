...
| PTX 9.3 `atom` / `red` | Supported subset | Synchronous scalar explicit-global `add/min/max` in `.u32/.s32`, plus `atom.cas.b32`, with legacy and explicit relaxed-CTA qualifier forms. See [atomic and reduction coverage](atomic_reduction_coverage.md). |
