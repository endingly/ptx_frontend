...
| PTX 9.3 `atom` / `red` | Supported subset | Synchronous scalar explicit-global 32-bit operations plus 64-bit `add.u64`, `min/max.u64/.s64`, and bitwise `.b64`; `atom` also supports `exch.b64` and `cas.b64`. Legacy and explicit relaxed-CTA forms are covered in [atomic and reduction coverage](atomic_reduction_coverage.md). |
