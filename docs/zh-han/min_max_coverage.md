...
这些 public shape change 在 installed C++ package `0.1.0` 曾明确构成 API break；其 CMake package compatibility 为 `SameMinorVersion`。不提供 compatibility shim 或并行的 legacy representation，因此 consumer 需要迁移到重命名后的 cohort 和 operand variant。Python wheel 版本为 `0.1.0b0`，属于 beta prerelease；installed C++ package 当前为 `0.2.0`。两者是独立的 package 版本，没有锁步要求。仓库没有专用 migration 或 changelog 文档，因此迁移说明保留在此处。
