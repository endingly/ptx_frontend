...
| PTX 9.3 `atom` / `red` | 支持子集 | 显式 global 的同步标量 `.u32/.s32` `add/min/max`、`.u32` `inc/dec`、`.b32` 位操作，以及 `atom.exch.b32` 和 `atom.cas.b32`；包含旧式和显式 relaxed-CTA 限定形式。详见[原子操作与归约覆盖范围](atomic_reduction_coverage.md)。 |
