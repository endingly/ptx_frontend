...
| Debug file directive | 支持子集 | outermost `.file file_index "filename"` 与可选且成对的 `, timestamp, file_size`；CST 无损、AST 保留 field range，duplicate index 与 `.loc` table 仍未支持 |
...
| 其他 directive | 尚未支持（直接拒绝） | `.loc`、section、pragma、module variable 与结构化 kernel-tuning directive；未建模 function-header token 不会静默进入 AST |
