...
| 恢复与编辑 | 仅基础设施 | 公开 parser/lowering result 已携带有序 `DiagnosticCollection`；在 missing token、recovery node、synchronization、多错误解析与 token edit 到来前，parser 仍 fail-fast 且只报告一条 diagnostic |
