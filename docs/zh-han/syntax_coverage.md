...
| Resolved opcode | 部分支持 | machine-readable manifest 是已建模 opcode slice 与 deferred 范围的权威来源。M12 common-kernel corpus 在 `sm_80`、`sm_90a`、`sm_100` 上对 60 个冻结 form 执行 parse、resolve 与 target-aware check；`setmaxnreg.inc.sync.aligned.u32` 有意仅限 `sm_90a`。inventory 条目对已实现冻结 slice 保持 partial，其 residual variant 在 M12 后 deferred；simulator execution 仍 unsupported。 |
