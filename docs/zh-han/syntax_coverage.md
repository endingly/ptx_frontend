...
| 其他 directive | 部分支持 | 同 module `.alias` 会 canonicalize direct-call ABI lookup；建模 typed `.managed/.unified` attribute 与列出的 header directive。LD/ST 检查 unified-address 与只读约束；linker/backend 行为及运行时分配仍不支持 |
