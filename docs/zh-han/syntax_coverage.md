...
| 结构化控制语法 | 支持子集 | `.callprototype` 已有专用的 function-local CST/AST grammar，并保留 PTX 9.3 signature/suffix payload；nested scope、`.calltargets`/`.branchtargets` 以及全部 metadata binding、declaration semantics、resolution 和 control-flow checking 仍未支持 |
