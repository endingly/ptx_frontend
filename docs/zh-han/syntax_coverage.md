...
| PTX 9.3 `atom` / `red` | 支持子集 | 显式 global 的同步标量 32 位操作，以及 64 位 `add.u64`、`min/max.u64/.s64` 和 `.b64` 位操作；`atom` 还支持 `exch.b64` 与 `cas.b64`。两者都支持浮点 `.add.f32/.f64`。旧式和显式 relaxed-CTA 形式详见[原子操作与归约覆盖范围](atomic_reduction_coverage.md)。 |
