...
def generate_resolved_ir_header(
    ...
    *,
    ...
    """Generate the public Resolved IR declaration fragment.

    The fragment is intended to be included by ``ptx_resolved_ir.hpp`` after
    ``WithLocs``, resolved operand primitives, and ``check_end`` descriptors
    have been declared.  It therefore deliberately does not include headers
    or open a namespace of its own.
    """

    instructions = tuple(
        from_instruction_spec(bound_instruction.spec)
...
// This declaration fragment must be included inside
