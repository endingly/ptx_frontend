    ...
    *,
    ...
    for entry in context.entries:
        ...
        storage_definitions.append(
            _emit_instruction_descriptor_storage(
                descriptor, context.backend, cpp_name=entry.cpp_name
            )
        )
        getter_definitions.append(
            _emit_instruction_descriptor_getter(cpp_name=entry.cpp_name)
        )
...
def emit_check_end_instruction_descriptor_implementation(
    ...
    *,
    ...
    ``cpp_name`` must come from a validated ``GenerationInstruction`` binding.
    """

    return (
        f"{_emit_instruction_descriptor_storage(descriptor, backend, cpp_name=cpp_name)}\n\n"
        f"{_emit_instruction_descriptor_getter(cpp_name=cpp_name)}"
    )


def _emit_instruction_descriptor_storage(
    descriptor: SyntaxInstructionDescriptor, backend: CodegenUnit, *, cpp_name: str
) -> str:
    """Emit the anonymous-namespace storage for one instruction descriptor."""

    storage_name = f"{cpp_name}DescriptorStorage"
    variant_storage = "\n\n".join(
...
def _emit_instruction_descriptor_getter(*, cpp_name: str) -> str:
...
{cpp_name}::get_syntax_descriptor() noexcept {{
