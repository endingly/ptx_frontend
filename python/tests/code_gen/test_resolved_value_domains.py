...
class ResolvedValueDomainGenerationTests(unittest.TestCase):
    def test_generates_marked_backend_domains_as_constexpr_tables(self) -> None:
        ...
        self.assertIn('{"ne", ComparisonOperator::Ne}', source)
        self.assertIn('{"lo", ComparisonOperator::Lo}', source)
        self.assertIn('{"nan", ComparisonOperator::Nan}', source)
        self.assertIn("std::array<PtxSuffixEntry<TestProperty>, 6>", source)
        self.assertIn("kTestProperties", source)
        self.assertIn('{"finite", TestProperty::Finite}', source)
        self.assertIn('{"subnormal", TestProperty::Subnormal}', source)
