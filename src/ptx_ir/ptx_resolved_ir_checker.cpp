...
CheckResult check_operands(std::span<const OperandDescriptor> descriptors,
  ...
  for (const OperandDescriptor& descriptor : descriptors) {
    ...
    if (descriptor.type_expr.empty() || !descriptor.type_expr.starts_with('$'))
      continue;

    const std::string_view type_field_id = descriptor.type_expr.substr(1);
    const FieldView* type_field = find_field(fields, type_field_id);
    if (type_field == nullptr || !type_field->scalar_type) {
      diagnostics.push_back(CheckDiagnostic{
          ...
          .message = fmt::format(
              ...
              descriptor.target_field_id, type_field_id),
      });
      continue;
    }

    if (operand->immediate_type &&
        *operand->immediate_type != *type_field->scalar_type) {
      diagnostics.push_back(CheckDiagnostic{
          ...
          .message = fmt::format(
              ...
              type_field_id, to_string(*type_field->scalar_type)),
      });
    }
  }
