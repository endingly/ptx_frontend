...
.entry kernel() { .reg .pred %p0; .reg .b32 %r0, %r1; setp.ge.s32 %p0, %r0, %r1; }
  ...
  const auto valid = resolveModule(*parsed_module_1);
  ASSERT_TRUE(valid.has_value()) << valid.error().front().message;
  const auto& instruction = std::get<Setp>(valid->functions.front().body.front());
...
.entry kernel() { .reg .pred %p0; .reg .b64 %rd0; .reg .b32 %r1; setp.ge.s32 %p0, %rd0, %r1; }
  ...
  const auto invalid = resolveModule(*parsed_module_2);
  ASSERT_TRUE(invalid.has_value()) << invalid.error().front().message;
  const auto& invalid_instruction = std::get<Setp>(invalid->functions.front().body.front());
...
.entry kernel() {
  ...
  ASSERT_MODULE_PARSE_SUCCEEDS(parsed_module_3);
  EXPECT_FALSE(resolveModule(*parsed_module_3)
                   .has_value());
}
...
.entry kernel() { .reg .f32 %dst, %src2, %src3; .reg .u32 %wrong_src; fma.rn.f32 %dst, %wrong_src, %src2, %src3; }
  ...
  const auto& instruction = std::get<Fma>(wrong_scalar->functions.front().body.front());
  const auto& variant = std::get<Fma::RnF32>(instruction.variant);
  const auto checked = checker::check(
      instruction, checker::Context{.target = {.ptx_version = {2, 0}, .sm_version = 20}});
  ASSERT_FALSE(checked.has_value());
  EXPECT_EQ(checked.error().front().kind, checker::CheckDiagnosticKind::OperandTypeMismatch);
  EXPECT_EQ(checked.error().front().range, variant.src1.locs.front());
...
.entry kernel() { .reg .b64 %dst, %src1, %src3; .reg .b32 %wrong_src; fma.rn.f32x2 %dst, %src1, %wrong_src, %src3; }
  ...
  const auto& packed_instruction = std::get<Fma>(wrong_packed->functions.front().body.front());
  ...
  EXPECT_EQ(packed_checked.error().front().kind,
            checker::CheckDiagnosticKind::OperandTypeMismatch);
  EXPECT_EQ(packed_checked.error().front().range, packed_variant.src2.locs.front());
}
...
.entry kernel() {
  ...
  for (size_t index = 0; index != body.size(); ++index) {
    EXPECT_TRUE(checker::check(std::get<Fma>(body[index]), contexts[index]).has_value())
        << index;
  }
...
.entry kernel() { .reg .b16 %dst, %src2, %src3; .reg .f16 %wrong_src; fma.rn.bf16 %dst, %wrong_src, %src2, %src3; }
  ...
  const auto& bfloat_instruction = std::get<Fma>(wrong_bfloat->functions.front().body.front());
  ...
  EXPECT_EQ(bfloat_checked.error().front().kind,
            checker::CheckDiagnosticKind::OperandTypeMismatch);
  EXPECT_EQ(bfloat_checked.error().front().range, bfloat_variant.src1.locs.front());
...
.entry kernel() { .reg .f32 %dst; .reg .b16 %src1; .reg .f16 %src2; .reg .b32 %src3; fma.rn.f32.bf16 %dst, %src1, %src2, %src3; }
  ...
  const auto& mixed_instruction = std::get<Fma>(wrong_mixed->functions.front().body.front());
  const auto& mixed_variant = std::get<Fma::MixedF32Bf16>(mixed_instruction.variant);
  ...
  EXPECT_EQ(mixed_checked.error().front().kind,
            checker::CheckDiagnosticKind::OperandTypeMismatch);
  EXPECT_EQ(mixed_checked.error().front().range, mixed_variant.src2.locs.front());
}
...
TEST(ResolvedModule, ChecksM12PrmtRegisterWidths) {
  const auto parsed_module_1 = parseModule(".entry kernel() { .reg .u32 %r0, %r1, %r2, %r3; prmt.b32 %r0, %r1, %r2, 0x5410; prmt.b32.f4e %r0, %r1, %r2, %r3; }");
  ASSERT_MODULE_PARSE_SUCCEEDS(parsed_module_1);
  const auto valid = resolveModule(*parsed_module_1);
  ASSERT_TRUE(valid.has_value()) << valid.error().front().message;
  ...
  const auto parsed_module_2 = parseModule(".entry kernel() { .reg .u16 %r0; .reg .u32 %r1, %r2; prmt.b32 %r0, %r1, %r2, 0; }");
  ASSERT_MODULE_PARSE_SUCCEEDS(parsed_module_2);
  const auto wrong = resolveModule(*parsed_module_2);
  ASSERT_TRUE(wrong.has_value()) << wrong.error().front().message;
...
TEST(ResolvedModule, ChecksM12PopcTypes) {
  const auto context = checker::Context{.target = {.ptx_version = {2, 0}, .sm_version = 20}};
  const auto parsed_module_1 = parseModule(".entry kernel() { .reg .u32 %dst, %src; popc.b32 %dst, %src; }");
  ASSERT_MODULE_PARSE_SUCCEEDS(parsed_module_1);
  const auto valid = resolveModule(*parsed_module_1);
  ASSERT_TRUE(valid.has_value()) << valid.error().front().message;
...
TEST(ResolvedModule, ChecksM12ClzTypes) {
  const auto context = checker::Context{.target = {.ptx_version = {2, 0}, .sm_version = 20}};
  const auto parsed_module_1 = parseModule(".entry kernel() { .reg .u32 %dst, %src32; .reg .u64 %src64; clz.b32 %dst, %src32; clz.b64 %dst, %src64; }");
  ASSERT_MODULE_PARSE_SUCCEEDS(parsed_module_1);
  const auto valid = resolveModule(*parsed_module_1);
  ASSERT_TRUE(valid.has_value()) << valid.error().front().message;
...
TEST(ResolvedModule, ChecksM12BfindTypes) {
  const auto context = checker::Context{.target = {.ptx_version = {2, 0}, .sm_version = 20}};
  const auto parsed_module_1 = parseModule(".entry kernel() { .reg .u32 %dst, %src; bfind.shiftamt.u32 %dst, %src; }");
  ASSERT_MODULE_PARSE_SUCCEEDS(parsed_module_1);
  const auto valid = resolveModule(*parsed_module_1);
  ASSERT_TRUE(valid.has_value()) << valid.error().front().message;
...
TEST(ResolvedModule, ChecksM12BrevTypes) {
  const auto context = checker::Context{.target = {.ptx_version = {2, 0}, .sm_version = 20}};
  const auto parsed_module_1 = parseModule(".entry kernel() { .reg .u32 %dst, %src; brev.b32 %dst, %src; }");
  ASSERT_MODULE_PARSE_SUCCEEDS(parsed_module_1);
  const auto valid = resolveModule(*parsed_module_1);
  ASSERT_TRUE(valid.has_value()) << valid.error().front().message;
