...
.entry kernel() { mbarrier.expect_tx.shared.b64 [shared_value], 4294967296; }
  ...
  EXPECT_EQ(
      out_of_range_tx_count.error().front().message,
...
.entry kernel() { .reg .b64 %state; .reg .u64 %rd0;
  ...
  EXPECT_NE(
      cluster_register.error().front().message.find("requires the '_' sink"),
      std::string::npos);

  for (const std::string_view source : {
           ".shared .align 8 .b64 shared_value; .entry kernel() { "
           "mbarrier.arrive.expect_tx.release.cta.shared.b64 _, "
           "[shared_value], 4294967296; }",
           ".shared .align 8 .b64 shared_value; .entry kernel() { "
           "mbarrier.arrive.expect_tx.release.cluster.shared.b64 _, "
           "[shared_value], 4294967296; }",
       }) {
    SCOPED_TRACE(source);
    ...
    EXPECT_EQ(
        out_of_range.error().front().message,
        "Integer literal '4294967296' is out of range for scalar type 'U32'.");
  }
}
...
TEST(ResolvedModule, ChecksMbarrierArrivalCountRangesInEveryCountForm) {
  constexpr std::array forms{
      ...
      "mbarrier.arrive.release.cluster.shared::cluster.b64 _, [barrier], "
      "$count;",
      ...
      "mbarrier.arrive.noComplete.release.cta.shared::cta.b64 _, [barrier], "
      "$count;",
      ...
      "mbarrier.arrive_drop.release.cluster.shared::cluster.b64 _, [barrier], "
      "$count;",
...
.entry kernel() { .reg .pred %p0; .reg .b64 %state;
  ...
  EXPECT_EQ(
      out_of_range_time_hint.error().front().message,
      "Integer literal '4294967296' is out of range for scalar type 'U32'.");
}
...
.entry kernel() {
  ...
  for (const std::string_view source : {
           ".entry kernel() { .reg .u32 %r<2>; mapa.shared::cluster.u32 %r0, "
           "0, 0; }",
           ".entry kernel() { .reg .u32 %r<2>; mapa.shared::cluster.u32 %r0, "
           "%tid.x, 0; }",
           ".global .u32 value; .entry kernel() { .reg .u32 %r<2>; mapa.u32 "
           "%r0, value, 0; }",
           ".entry kernel() { .reg .u32 %r<2>; mapa.u32 %r0, %r1+4, 0; }",
           ".func device() {} .entry kernel() { .reg .u32 %r<2>; "
           "mapa.shared::cluster.u32 %r0, device, 0; }",
       }) {
    SCOPED_TRACE(source);
    const auto parsed_module_3 = parseModule(source);
    ASSERT_MODULE_PARSE_SUCCEEDS(parsed_module_3);
...
.entry kernel() {
  ...
  for (const std::string_view source : {
           ".entry kernel() { .reg .u32 %r<2>; getctarank.shared::cluster.u32 "
           "%r0, 0; }",
           ".entry kernel() { .reg .u32 %r<2>; getctarank.shared::cluster.u32 "
           "%r0, %tid.x; }",
           ".global .u32 value; .entry kernel() { .reg .u32 %r<2>; "
           "getctarank.u32 %r0, value; }",
           ".entry kernel() { .reg .u32 %r<2>; getctarank.u32 %r0, %r1+4; }",
           ".func device() {} .entry kernel() { .reg .u32 %r<2>; "
           "getctarank.shared::cluster.u32 %r0, device; }",
       }) {
    SCOPED_TRACE(source);
    const auto parsed_module_3 = parseModule(source);
    ASSERT_MODULE_PARSE_SUCCEEDS(parsed_module_3);
  ...
  for (const std::string_view source : {
           ".entry kernel() { .reg .u32 %r<2>; .reg .u64 %rd0; getctarank.u32 "
           "%rd0, %r0; }",
...
.entry kernel() { .reg .u32 %u0; .reg .pred %p0; .reg .b64 %d0; elect.sync %u0|%p0, %d0; }
  ...
  for (const std::string_view source : {
           ...
           ".entry kernel() { .reg .u32 %u0; .reg .pred %p0; elect.sync %u0, "
           "0; }",
           ".entry kernel() { .reg .u32 %u0; .reg .pred %p0; elect.sync "
           "%u0|%p0; }",
       }) {
    SCOPED_TRACE(source);
    const auto parsed_module_4 = parseModule(source);
